...
- O(n^2) worst (reverse-sorted), O(1) extra space, best-in-class constants for small n -- sequential access, branch-light.

## Approach -- shift-and-insert
...
- Locate the insertion point with binary search over the sorted prefix instead of scanning: comparisons drop from O(n^2) to O(n log n).
- The shifts are still O(n^2) total, so it only pays off when comparisons are expensive (long strings, costly comparators).

## Alternative -- Shell sort (gapped insertion)

- Insertion sort over gapped subsequences with a decreasing gap sequence (Shell, Knuth, Sedgewick, ...).
- A good gap sequence upgrades it to roughly O(n^1.3) -- still not O(n log n), but a big step up from naive insertion.

## Complexity

- Time: O(n^2) worst and average, O(n) on already-sorted input.
- Space: O(1).
...
- Any "small array, near-sorted, or online" sorting need.

## Cousins & contrasts

- **Bubble sort**: same adjacent-inversion idea but swaps instead of shifts -- 2x the writes, no compensating advantage.
...
- **Merge / quick sort**: the O(n log n) options; insertion sort is the winner only at small n or on nearly-sorted data.
