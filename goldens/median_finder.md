...
- The whole game is one invariant: **every element in `lo` ≤ every element in `hi`**. Then the top(s) are the middle element(s) by construction.
...
- **min_heap**: the building block — this kata is two heaps glued face-to-face.
- **kth_largest**: median is the special case k = n/2, but quickselect needs the whole array up front and mutates it — useless on a live stream.
