...
int main() {
    { TEST("mixed_temps");
        ...
        auto _r = solve(_arr);
        { auto _exp = vector<int>{1, 1, 4, 2, 1, 1, 0, 0}; CHECK(_r == _exp); }
    }

    { TEST("monotonic_increasing");
        vector<int> _arr = {30, 40, 50, 60};
        auto _r = solve(_arr);
        { auto _exp = vector<int>{1, 1, 1, 0}; CHECK(_r == _exp); }
    }

    { TEST("short_increasing");
        vector<int> _arr = {30, 60, 90};
        auto _r = solve(_arr);
        { auto _exp = vector<int>{1, 1, 0}; CHECK(_r == _exp); }
    }

    { TEST("monotonic_decreasing");
        vector<int> _arr = {90, 80, 70};
        auto _r = solve(_arr);
        { auto _exp = vector<int>{0, 0, 0}; CHECK(_r == _exp); }
    }
